...
The core objective of this framework is to classify track quality using precise measurements, isolate true signal conversion electrons ($CE$), and reject high-error systematic noise and tracking fragments under heavy detector calibration stress.
...
The codebase has been split into dedicated, independent research chapters to prevent memory leaks and variable collisions:

*   **`00_TrkQualTrain_main.ipynb`**: The master training pipeline. Handles raw feature loading from ROOT files, balances sample weights, tunes tree hyperparameters, trains the final 500-tree BDT ensemble, and serializes the weights to storage.
*   **`01_data_perturbation.ipynb`**: The file that is responsible for creating and perturbing all of the required data.
*   **`02_factive_nactive_analysis.ipynb`**: Investigates the physical properties of track hits. Evaluates feature correlations and isolates the impact of fractional ratios like `factive`  and `nactive` on the model's background rejection capabilities.
...
###  Key Physics Insights From Synthesis
*   **Signal Recovery:** Under an extreme `momerr +50%` tracking penalty, a static cut discards vital tracks due to model skepticism. Shifting the threshold adaptively to `0.7915` successfully recovers **+13,511 pristine signal events** that would have been permanently lost.
*   **Background Security:** Loosening the criteria for noisy files to rescue signal tracks does not flood the analysis with noise. Even under extreme distortions, final background track changes are held safely between **19 and 51 tracks**, proving the classifier's structural integrity.
